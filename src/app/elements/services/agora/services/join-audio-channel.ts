...
export class JoinAudioChannel implements IAudioJoinChannel<IAudioTrack>{
    ...
    public WithMediaStream(audioMediaStream: MediaStreamTrack): IJoinChannelApply<IAudioTrack> {
        this.localAudioTrack = AgoraRTC.createCustomAudioTrack({ mediaStreamTrack: audioMediaStream, encoderConfig: 'high_quality' });
        return this;
    }

    public WithMicrophone(microphoneId: string): IJoinChannelApply<IAudioTrack> {
        this.requestInWait = AgoraRTC.createMicrophoneAudioTrack({ microphoneId: microphoneId, encoderConfig: 'high_quality', AEC: true, ANS: true, AGC: false });
        return this;
    }
    ...
    public async Apply(): Promise<IAudioTrack> {
        ...
        else { localTrack = await AgoraRTC.createMicrophoneAudioTrack({ encoderConfig: 'high_quality', AEC: true, ANS: true, AGC: false }); }
